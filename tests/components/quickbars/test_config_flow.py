"""Tests for the QuickBars config flow."""
...
@dataclass
class _ZCStub:
    """Minimal stand-in for ZeroconfServiceInfo for tests."""
...
async def test_zeroconf_discovery_to_confirm(
    hass: HomeAssistant, patch_client_all
) -> None:
    ...
    result = await hass.config_entries.flow.async_configure(
        result["flow_id"], user_input={"code": "9999"}
    )
    ...
    if result["type"] == "form":
        ...
        result = await hass.config_entries.flow.async_configure(
            result["flow_id"],
            user_input={"url": "http://ha.local:8123", "token": "abc123"},
        )
        assert result["type"] == "create_entry"
